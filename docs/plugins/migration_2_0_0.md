...
The bundled default terminal filters have already been updated accordingly, with regular expressions that match both the old and the new prefix so they keep working on logs from any third party plugins that might still emit the legacy format.

The table below lists the default terminal filters in their old and new form.

| Name | Old default regex | OctoPrint 2.0.0 default regex |
| --- | --- | --- |
...
The Global API Key has been deprecated for a while, with 2.0.0 it is no longer automatically generated at startup and may thus be empty! It has been removed entirely in 2.1.0.

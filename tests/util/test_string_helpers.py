...
class StringHelperTest(unittest.TestCase):
    ...
    def test_to_str_bytes(self):
        result = octoprint.util.to_str(b"test")
        self.assertEqual(result, "test")
        self.assertIsInstance(result, str)

    def test_to_str_bytes_utf8(self):
        data = "äöüß"
        result = octoprint.util.to_str(data.encode("utf-8"), encoding="utf-8")
        self.assertEqual(result, data)
        self.assertIsInstance(result, str)

    def test_to_str_bytes_utf8_vs_ascii(self):
        self.assertRaises(
            UnicodeDecodeError,
    ...
    def test_to_str_bytes_utf8_vs_ascii_replace(self):
        data = "äöüß"
        result = octoprint.util.to_str(

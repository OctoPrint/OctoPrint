...
context('Test settings dialog opens and closes', () => {
    ...
    it('opens settings', () => {
        cy.get('[data-test-id=settings-open]')
            .click();
        cy.get('[data-test-id=settings-dialog]')
            .should('be.visible');
    });
    ...
    describe('close settings', () => {
        ...
        it('closes settings via button', () => {
            cy.get('[data-test-id=settings-close-button')
                .click();
        });

        it('closes settings via x', () => {
            cy.get('[data-test-id=settings-close-x')
                .click({ force: true });
        });
        ...
        it.skip('closes settings via ESC', () => {
            cy.get('[data-test-id=settings-dialog]')
                .trigger('keydown', { key: 'Escape', which: 27, code: 'Escape' });
        });

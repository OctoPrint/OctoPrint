		...
		def apply_filter(nodes, filter_func):
			...
			for key, node in nodes.items():
				if filter_func(node) or node["type"] == "folder":
					if node["type"] == "folder":
						node = copy.copy(node)
						node["children"] = apply_filter(node.get("children", []), filter_func)
					result[key] = node
			return result
		...
		def enrich_folders(nodes):
			...
			for key, value in nodes.items():
				if value["type"] == "folder":
					...
					value["children"] = self._list_folder(os.path.join(path, key),
					                                      base=value["path"] + "/",
	...
	def _get_metadata(self, path, force=False):
		...
		if isinstance(metadata, dict):
			...
			if new_size != old_size:
				self._logger.info("Deleted {} stale or invalid entries from metadata for path {}".format(old_size - new_size,
				                                                                                         path))
				self._save_metadata(path, metadata)
			else:
				with self._get_metadata_lock(path):

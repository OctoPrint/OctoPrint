...
class CuraPlugin(octoprint.plugin.SlicerPlugin,
	...
	def _is_new_engine_version(self, executable):
		...
		if p.returncode != 0:
			...
			self._logger.info("Assuming pre 15.06 version of CuraEngine.")
			return False

		for line in p.stderr.read().split('\n'):
			if "unknown option: j" in line.strip().lower():
				self._logger.info("Using pre 15.06 version of CuraEngine.")
				return False
		self._logger.info("Using post 15.06 version of CuraEngine.")
		return True

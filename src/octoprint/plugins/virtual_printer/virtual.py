...
class VirtualPrinter(object):
	...
	def read(self, size=1):
		...
		return self.outgoing.read(size=size, timeout=timeout)

	@property
	def in_waiting(self):
		return self.outgoing.in_waiting
	...
	def _send(self, line):
		...
		try:
			self.outgoing.write(to_bytes(line, encoding="ascii", errors="replace"))
		except Exception:
			if self.outgoing is None:
				pass
			else:
	...
	def read(self, size=1, timeout=0):
		...
		while len(result) < size:
			if self.in_waiting >= size:
				with self._mutex:
					result.extend(self._outgoing[:size])
					self._outgoing = self._outgoing[size:]

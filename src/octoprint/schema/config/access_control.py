...
class AccessControlConfig(BaseModel):
    ...
    **DO NOT TOUCH!** If changed you will no longer be able to log in with your existing accounts.
    ...
    match OctoPrint accounts, by setting this to ``true`` users will be logged into OctoPrint as the user during Basic Authentication.
    ...
    **ONLY ENABLE THIS** if your OctoPrint instance is only accessible through a connection locked down through an authenticating reverse proxy!
    ...
    OctoPrint accounts, by setting this to ``true`` the user's groups will be set to the groups provided in the header.

...
from octoprint.comm.protocol.reprap.util.queues import ScriptQueue, JobQueue, CommandQueue, SendQueue, QueueMarker, SendQueueMarker
...
class ReprapGcodeProtocol(Protocol, Fdm3dPrinterProtocolMixin, MotorControlProtocolMixin,
		...
		return [GroupChoiceType("flavor",
		                        ...
		                        ParamGroup("flavor_overrides",
		                                   ...
		                                   [SmallChoiceType("send_checksum",
		                                    ...
		                                    SmallChoiceType("trigger_ok_after_resend",
		                                                    ...
		                                                    [Value("detect", title=gettext("If detected as necessary")),
		                                                     Value("always", title=gettext("Always")),
		                                                     Value("never", title=gettext("Never"))],
		                                                    expert=True,
		                                                    help=gettext("Some Marlin forks lack an acknowledging `ok` with their resend requests. Set this to \"Always\" or \"If detected as necessary\" if you run into communication stalls on resend requests.")),
		                                    BooleanType("unknown_requires_ack",
		                                                gettext("Unknown commands require `ok`")),
		                                    BooleanType("unknown_with_checksum",
		                                                gettext("Send unknown commands with line number & checksum")),
		                                    BooleanType("detect_external_heatups",
		                                                gettext("Try to detect externally triggered blocking heatups")),
		                                    BooleanType("block_while_dwelling",
		                                                gettext("Block all communication while a dwell command (`G4`) is active")),
		                                    BooleanType("sd_relative_path",
		                                                gettext("Select SD files by relative path")),
		                                    BooleanType("sd_always_available",
		                                                gettext("Always assume that the SD card is present")),
		                                    BooleanType("heatup_abortable",
		                                                gettext("Allow cancelling blocking heatups if the firmware supports it")),
		        ...
		        ParamGroup("error_handling",
		                   ...
		                   [SmallChoiceType("firmware_errors", gettext("What to do on a firmware error (`Error:` or `!!`)"),
		                    ...
		                    BooleanType("send_m112",
		                                gettext("Send `M112` on disconnect due to error"),
		                                default=True,
		                                warning=False,
		                                help=gettext("If this is checked, OctoPrint will try to send an `M112` prior to a disconnect due to a (fatal) error in order to disable heaters and motors."))],
		                   advanced=True),
		        ParamGroup("pausing",
		                   gettext("Pausing"),
	...
	def _cancel_preparation_done(self, check_timer=True, suppress_script=False, user=None):
		if user is None:
	...
	def cancel_processing(self, error=None, user=None, tags=None, log_position=True):
		...
		if self.flavor.heatup_abortable:
			self.send_commands(self.flavor.command_abort_heatup(),
			                   part_of_job=True,
			                   tags=tags | {"trigger:comm.cancel",
	...
	def _pause_preparation_done(self, check_timer=True, suppress_script=False, user=None):
		...
		with self._pause_mutex:
			...
			self.notify_listeners("on_protocol_job_paused", self, self._job,
	...
	def pause_processing(self, user=None, tags=None, log_position=True, local_handling=True):
		if self._job is None or self.state not in (ProtocolState.PROCESSING,
	...
	def _continue_sending(self):
		...
		while self._active:
			job_active = self.state in (ProtocolState.STARTING, ProtocolState.PROCESSING) and not (self._job is None or not self._job.active or isinstance(self._job, SDFilePrintjob))

...
class SlicingProfile:
    ...
    def __init__(
        self, slicer, name, data, display_name=None, description=None, default=False
    ):
        self.slicer = slicer
        self.name = name
        self.data = data
...
class TemporaryProfile:
    ...
    def __enter__(self):
        ...
        temp_profile = tempfile.NamedTemporaryFile(
            prefix="slicing-profile-temp-", suffix=".profile", delete=False
        )
        temp_profile.close()
...
class SlicingManager:
    ...
    def reload_slicers(self):
        ...
        available slicers.
        """
        plugins = octoprint.plugin.plugin_manager().get_implementations(
        ...
        for plugin in plugins:
            try:
                ...
                self._logger.exception(
                    "Error while getting properties from slicer {}, ignoring it".format(
                        plugin._identifier
                    ),
    ...
    def slice(
        ...
        def slicer_worker(
            ...
            try:
                ...
                with self._temporary_profile(
                    slicer_name, name=profile_name, overrides=overrides
                ) as profile_path:
        ...
        slicer_worker_thread = threading.Thread(
            target=slicer_worker,
            args=(
                slicer,
                source_path,
                dest_path,
    ...
    def save_profile(
        ...
        self._save_profile_to_path(
            slicer, path, profile, overrides=overrides, allow_overwrite=allow_overwrite
        )

        payload = {"slicer": slicer, "profile": name}
        event = (
            octoprint.events.Events.SLICING_PROFILE_MODIFIED
            if is_overwrite
            else octoprint.events.Events.SLICING_PROFILE_ADDED
        )
    ...
    def _temporary_profile(self, slicer, name=None, overrides=None):
        ...
        return TemporaryProfile(
            self.get_slicer(slicer).save_slicer_profile, profile, overrides=overrides
        )
    ...
    def delete_profile(self, slicer, name):
        ...
        try:
            ...
            octoprint.events.eventManager().fire(
                ...
                {"slicer": slicer, "profile": name},
            )

    def set_default_profile(
        self, slicer, name, require_configured=False, require_exists=True
    ):
        """
    ...
    def all_profiles(self, slicer, require_configured=False):
        ...
        return self.get_slicer(slicer, require_configured=False).get_slicer_profiles(
            slicer_profile_path
        )
    ...
    def profiles_last_modified(self, slicer):
        ...
        slicer_profile_path = self.get_slicer_profile_path(slicer)
        return self.get_slicer(
            slicer, require_configured=False
        ).get_slicer_profiles_lastmodified(slicer_profile_path)
    ...
    def _save_profile_to_path(
        ...
        self.get_slicer(
            ...
            path, profile, allow_overwrite=allow_overwrite, overrides=overrides
        )
